...
func TestClientAttachDocument(t *testing.T) {
	...
	resp, err := client.AttachDocument(ctx, &types.ScannedDocumentRequest{CaseReference: "5"})
	assert.Nil(t, err)
	assert.Equal(t, &types.ScannedDocumentResponse{UUID: "hello"}, resp)
}
...
func TestClientCreateCaseStub(t *testing.T) {
	...
	resp, err := client.CreateCaseStub(ctx, &types.ScannedCaseRequest{BatchID: "1"})
	assert.Nil(t, err)
	assert.Equal(t, &types.ScannedCaseResponse{UID: "hello"}, resp)
}
