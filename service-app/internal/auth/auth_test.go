...
func TestAuthAuthenticate(t *testing.T) {
	...
	user, token, err := auth.Authenticate(w, r)
	assert.NoError(t, err)
	assert.Equal(t, User{Email: "john.doe@example.com", Password: "not-a-password"}, user)
...
func TestAuthUseCheckAuth(t *testing.T) {
	w := httptest.NewRecorder()
	r, _ := http.NewRequest(http.MethodGet, "", strings.NewReader(`{"user":{"email":"john.doe@example.com","password":"not-a-password"}}`))
	...
	auth.UseCheckAuth(handler)(w, r)

	resp := w.Result()
	assert.Equal(t, http.StatusTeapot, resp.StatusCode)
}

func TestAuthUseCheckAuth_InvalidToken(t *testing.T) {
	w := httptest.NewRecorder()
	r, _ := http.NewRequest(http.MethodGet, "", strings.NewReader(`{"user":{"email":"john.doe@example.com","password":"not-a-password"}}`))
	r.AddCookie(&http.Cookie{Name: cookieName, Value: "a-token"})
