...
func checkFile(token, fileType, fileName, id string) error {
	exec.Command("docker", "compose", "exec", "localstack", "awslocal", "sqs", "purge-queue", "--queue-url=ddc.fifo").Run()
	...
	cmdOut, _ := exec.Command("docker", "compose", "exec", "localstack", "awslocal", "sqs", "receive-message", "--queue-url=ddc.fifo").Output()
	var cv struct {
		Messages []struct {
			Body string
		}
